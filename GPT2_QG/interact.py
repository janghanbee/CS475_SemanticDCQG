...
from .train import build_hotpot_input_from_segments
...
def sample_sequence(inst, tokenizer, model, args, para_cache):
    ...
    instance, _ = build_hotpot_input_from_segments(inst, tokenizer, args.data_type, no_para=True, with_eos=False)  # as this is used for generate, with_eos is False.
...
def run():
    ...
    parser.add_argument("--subq_dir", type=str, default="", help="intermediate question's dir")
    ...
    # change intermediate question 
    if args.data_type == "hotpot_comp":
        newdata = []
        print("Loading immediate questions ...")
        with open(args.subq_dir, "r", encoding="utf-8") as f:
            intermediate_output = json.load(f)
            for inst in data:
                try:
                    inst['question1'] = tokenizer.encode(intermediate_output[inst['inst_id']])
                    newdata.append(inst)
                except:
                    print('id not found')
                    continue
    ...
    for inst in tqdm.tqdm(data):  # !!! each inst contains one different paragraph
        ...
        with torch.no_grad():
            ...
            if para_index != para_cache["index"]:
                ...
                instance, _ = build_hotpot_input_from_segments(inst, tokenizer, args.data_type, para_only=True) # !!!! this is designed to re-use paragraph.
                input_ids = torch.tensor(instance['input_ids'], device=args.device).unsqueeze(0)
                token_type_ids = torch.tensor(instance['token_type_ids'], device=args.device).unsqueeze(0)
        ...
        intermediate_output[inst['inst_id']] = generated_question
    ...
    if args.data_type == "hotpot_sub":
        with open(args.output_file[:-5]+".intermediate.json", "w", encoding="utf-8") as f:
    ...
    with open(args.output_file, "w", encoding="utf-8") as f:
        json.dump(final_output_dict, f, indent=4, ensure_ascii=False)
        print("saved generated questions.")
